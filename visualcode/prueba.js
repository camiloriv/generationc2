...
function listPaddockManagersByName() {
	let orden_nombres = paddockManagers.map((paddockManagers) => paddockManagers.name);
	orden_nombres.sort();
	let arreglo_rut = [];
	for (i = 0; i < paddockManagers.length; i++) {
		let obj_manager = paddockManagers.find(paddockManagers => orden_nombres[i] == paddockManagers.name)
		arreglo_rut.push(obj_manager);
	}
	let respuesta = arreglo_rut.map((resp) => resp.taxNumber)
	return respuesta;
};
...
function sortPaddockTypeByTotalArea() { // 2 1 3 4
	let areas = []
	for (i = 1; i <= paddockType.length; i++) {
		let parcela = paddocks.filter(paddock => paddock.paddockTypeId == i);
		let area_parcela = parcela.map((paddock) => paddock.area);
		let area_total_parcela = area_parcela.reduce((a, b) => a + b);
		let areas_totales = {
			obj_id_nombre: paddockType.find(paddockType => paddockType.id == i),
			area_total: area_total_parcela
		}
		areas.unshift(areas_totales)
	}
	areas.sort((a, b) => b.area_total - a.area_total);
	let respuesta = areas.map((paddock) => paddock.obj_id_nombre.name)
	return respuesta;
};
...
function sortFarmManagerByAdminArea() {
	...
	let total_areas = []
	for (i = 1; i <= paddockManagers.length; i++) {
		let filtro_area_manager = paddocks.filter(paddock => paddock.paddockManagerId == i);
		let areas_manager = filtro_area_manager.map((paddock) => paddock.area);
		let area_total_manager = areas_manager.reduce((a, b) => a + b);
		let nuevo_obj = {
			manager: paddockManagers.find(paddockManagers => paddockManagers.id == i),
			total_area: area_total_manager
		}
		total_areas.unshift(nuevo_obj)
		total_areas.sort((a, b) => b.total_area - a.total_area)
	}
	return total_areas.map((paddockManagers) => paddockManagers.manager.name)
};
...
function farmManagerNames() {
	...
	for (let farm of farms) {
		let managers = []
		let paddockFiltrado = paddocks.filter((paddock) => paddock.farmId == farm.id)
		if (paddockFiltrado.length >= 1) {
			for (let manager of paddockManagers) {
				for (let paddock of paddockFiltrado) {
					if (paddock.paddockManagerId == manager.id && !managers.includes(manager)) {
		...
		let sorted_managers = managers.sort((a, b) => {
			if (a.name < b.name) {
				return 1
			}
			if (a.name > b.name) {
		...
		respuesta[farm.name] = sorted_managers.map((manager) => manager.taxNumber)
	}
	return respuesta
};
...
function biggestAvocadoFarms() {
	let respuesta = []
	let paltos = paddocks.filter(m => m.paddockTypeId == 1)

	for (let farm of farms) {
		let campo = paltos.filter((palto) => palto.farmId == farm.id)
		let total_area_campo = campo.reduce((acc, area_campo) => {
			return acc += area_campo.area;
		}, 0)
		if (total_area_campo > 20000) {
			respuesta.push(total_area_campo)
		}
	}

	respuesta.sort((a, b) => b - a)
	return respuesta
}
...
function biggestCherriesManagers() {
	let forestal = paddocks.filter((nombre) => nombre.farmId == 3 && nombre.paddockTypeId == 3)
	let respuesta = []
	for (let datos of forestal) {
		if (datos.area > 1000) {
			let manager_name = paddockManagers.filter((manager) => manager.id == datos.paddockManagerId)
			for (let name of manager_name) {
				respuesta.push(name.name)
			}
		}
	}
	return respuesta
...
function farmManagerPaddocks() {
	...
	for (let manager of paddockManagers) {
		let arrFarms = []
		let paddockFiltrado = paddocks.filter((paddock) => paddock.paddockManagerId == manager.id)
		if (paddockFiltrado.length >= 1) {
			for (let farm of farms) {
				for (let paddock of paddockFiltrado) {
					if (paddock.farmId == farm.id && !arrFarms.includes(farm)) {
						arrFarms.push(farm)
					}
				}
			}
		...
		let sorted_arrFarms = arrFarms.sort((a, b) => {
			if (a.name < b.name) {
				return 1
			}
			if (a.name > b.name) {
		...
		respuesta[manager.name] = sorted_arrFarms.map((farmSorted) => farmSorted.name)
	}
	return respuesta
};
