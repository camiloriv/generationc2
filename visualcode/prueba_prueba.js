...
function listPaddockManagersByName() {
  let sorted_managers = paddockManagers.sort((a, b) => {
    if (a.name < b.name) {
      return 1
    }
    if (a.name > b.name) {
  ...
  let taxt_num_manager = []
  for (let manager of sorted_managers) {
    taxt_num_manager.push(manager.taxNumber)
  }
  return taxt_num_manager;
};
...
function sortPaddockTypeByTotalArea() { // 2 1 3 4
  ...
  for (i = 1; i <= paddockType.length; i++) {
    let farm = paddocks.filter(byId => paddock.paddockTypeId == i); // filtra objeto Paddock por id
    let area_parcela = parcela.map((paddock) => paddock.area); // deja un Array con las areas de ese Paddock
    let area_total_parcela = area_parcela.reduce((a, b) => a + b); // suma todas las areas de ese Paddock
    let areas_totales = { // guarda numero id y area total del paddock
      obj_id_nombre: paddockType.find(paddockType => paddockType.id == i), //  devuelve el objeto con el id tanto
      area_total: area_total_parcela
    }
    areas.push(areas_totales)
  }
...
function sortFarmManagerByAdminArea() {
  ...
  for (i = 1; i <= paddockManagers.length; i++) {
    let filtro_area_manager = paddocks.filter(paddock => paddock.paddockManagerId == i); // filtra los paddocks de un manager
...
function farmManagerNames() {
    let respuesta = []
  for (i = 1; i <= farms.length; i++) {
    let nombres = farms.map((farms) => farms.name);  // nombre de los campos
    let farm_filtro = paddocks.filter(paddock => paddock.farmId == i) // filtro por campo
    let id_manager = farm_filtro.map((paddock) => paddock.paddockManagerId) // manager que adm este campo
    let no_dupli = []
    id_manager.forEach((elemento) => {
      if (!no_dupli.includes(elemento)) { no_dupli.push(elemento) }
    });
    return
  }
};
